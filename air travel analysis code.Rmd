...
```{r echo = F, message = F, warning = F}
#2. ПРОВЕРИМ ГИПОТЕЗУ О ВЛИЯНИИ ВРЕМЕНИ ГОДА НА ОЦЕНКУ МЕСТ ХРАНЕНИЯ ПАССАЖИРОМ, РАССМОТРЕВ ОЦЕНКИ НА РАЗНЫХ КАТЕГОРИЯХ КАБИН - ТОПОВЫХ (БИЗНЕС-КЛАСС) И НЕ ТОПОВЫХ (ЭКОНОМ-КЛАСС)
...
lounge_ready = data.frame(country = lounge$author_country, recommended = lounge$recommended, num = lounge$only_good_num)
...
cool_per = str_c(rounded_percentage, '%')
...
*ответ на вопрос:* №3
\
*обоснование:* На графике очень наглядно показаны "грязные" аэропорты с наибольшим количеством "чистых" лаундж-зон. Данные можно легко и быстро интерпретировать, чтобы понять, какие аэропорты наиболее всего нуждаются в апгрейде в сфере чистоты.
...
*ответ на вопрос:* №3
\
*обоснование:* Общее количество лаундж-зон в датафрейме помогает 4му элементу дэшборда отразить критичность ситуации в сфере чистоты аэропортов.
...
*ответ на вопрос:* №3
\
*обоснование:* Таблица показывает список лаундж-зон, рейтинг чистоты которых выше рейтинга чистоты аэропортов, в которых они находятся, тем самым представляя собой не то иное, как ответ на поставленный вопрос (вопрос №3).
...
*ответ на вопрос:* №3
\
*обоснование:* Количество лаундж-зон, представленных таблицей (3им элементом дэшбэрда), в купе с общим количеством лаундж-зон датафрейма (2ым элементом дэшборда), показывает драматичность ситуации с чистотой в представленных графиком (1м элементом дэшборда) аэропортах. 



***(примечание: в дэшборде рассматривается именно третий вопрос потому, что, на мой взгляд, он наиболее практичен, и, ответив на него, можно прийти к полезным заключениям, сформулировать реальные советы по улучшению сервиса)***
